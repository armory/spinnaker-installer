...
resource "aws_iam_role" "SpinnakerInstanceProfile" {
    name = "${var.spinnaker_instance_profile}"
...
resource "aws_iam_role" "SpinnakerManagedProfile" {
    name = "${var.spinnaker_managed_profile}"
    assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_policy" "SpinnakerAccessPolicy" {
    name = "${var.spinnaker_access_policy}"
    policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_policy" "SpinnakerAssumeRolePolicy" {
    name = "${var.spinnaker_assume_policy}"
    policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_policy" "SpinnakerS3AccessPolicy" {
  name = "${var.spinnaker_s3_access_policy}"
  policy = <<EOF
{
    "Version": "2012-10-17",
    "Statement": [
